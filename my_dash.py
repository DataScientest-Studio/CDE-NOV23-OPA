...
def get_crypto_data():
    try:
        response = requests.get("http://web:8000/getcrypto")
        data = response.json()
...
def update_crypto():
    try:
        response = requests.get("http://web:8000/majcrypto")
...
def get_update_result():
    try:
        response = requests.get("http://web:8000/getmajcrypto")
        # print(response)
...
app.layout = html.Div([    
    ...
    dash_table.DataTable(
        ...
        data=get_crypto_data(),  # Initialisation avec les données non filtrées
...
def update_table(n_clicks):
    ...
    if n_clicks > 0:
        update_crypto()  # Appel de l'API pour mettre à jour les cryptos
...
def display_update_result(n_clicks):
    if n_clicks > 0:
        result = get_update_result()  # Appel de l'API pour obtenir les résultats
